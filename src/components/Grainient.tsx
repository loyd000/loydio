...
// ─── Theme hook ──────────────────────────────────────────────────────────────
// Watches the data-theme attribute on <html> so Grainient reacts to toggles.
function useIsDark() {
...
// ─── Keep renderer alive across re-renders ────────────────────────────────────
type GrainientCtx = {
...
interface GrainientProps {
  ...
  centerY?: number;
  zoom?: number;
  /** Light-mode colors (defaults provided) */
  lightColor1?: string;
  lightColor2?: string;
  lightColor3?: string;
  /** Dark-mode colors (defaults provided) */
  darkColor1?: string;
  darkColor2?: string;
  darkColor3?: string;
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  }, []); // WebGL context built once

  // Effect 2: sync all props + theme colors to uniforms (zero GPU cost)
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
